...
/**
 * print_magic - print magic nos of Elf header
 * @e_ident: pointer to array
 *
...
/**
 * print_abi - Print ABI version,(ELF header)
 * @e_ident: pointer to array
 */
void print_abi(unsigned char *e_ident)
{
...
/**
 * print_entry - Print entry point (ELF header)
 * @e_entry: entry point address of ELF
 * @e_ident: pointer to array
 */
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
 ...
 *
 * Description: exit code 98 (cannot close file)
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
...
/**
 * main - shows info on ELF header file
 * @argc: args count
 * @argv: args vect
 *
 * Return: success 0
 *
 * Description: exit code 98, no ELF file, function fails
 */
